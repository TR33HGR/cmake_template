...
set(cmake_template_WEB_DIR "${CMAKE_SOURCE_DIR}/web")
set(cmake_template_COI_WORKER "${cmake_template_WEB_DIR}/coi-serviceworker.min.js")
set(cmake_template_SHELL_TEMPLATE "${cmake_template_WEB_DIR}/shell_template.html.in")
set(cmake_template_INDEX_TEMPLATE "${cmake_template_WEB_DIR}/index_template.html.in")
...
if(EMSCRIPTEN)
  ...
  # Set WASM build flag
  set(cmake_template_WASM_BUILD ON CACHE BOOL "Building for WebAssembly" FORCE)

  # Sanitizers don't work with Emscripten
  foreach(sanitizer ADDRESS LEAK UNDEFINED THREAD MEMORY)
    set(cmake_template_ENABLE_SANITIZER_${sanitizer} OFF CACHE BOOL "Not supported with Emscripten")
  endforeach()

  # Disable static analysis and strict warnings for Emscripten builds
  foreach(option CLANG_TIDY CPPCHECK WARNINGS_AS_ERRORS)
    set(cmake_template_ENABLE_${option} OFF CACHE BOOL "Disabled for Emscripten")
  endforeach()
  ...
  set(cmake_template_WASM_INITIAL_MEMORY "33554432" CACHE STRING
      "Initial WASM memory in bytes (default: 32MB)")
  set(cmake_template_WASM_PTHREAD_POOL_SIZE "4" CACHE STRING
      "Pthread pool size for WASM builds (default: 4)")
  set(cmake_template_WASM_ASYNCIFY_STACK_SIZE "65536" CACHE STRING
...
function(cmake_template_configure_wasm_target target)
  if(EMSCRIPTEN)
    ...
    set_property(GLOBAL APPEND PROPERTY cmake_template_WASM_TARGETS "${target}")
    ...
    target_link_options(${target} PRIVATE
      ...
      "-sUSE_PTHREADS=1"
      "-sPROXY_TO_PTHREAD=1"
      "-sPTHREAD_POOL_SIZE=${cmake_template_WASM_PTHREAD_POOL_SIZE}"
      # Enable asyncify for emscripten_sleep and async operations
      "-sASYNCIFY=1"
      "-sASYNCIFY_STACK_SIZE=${cmake_template_WASM_ASYNCIFY_STACK_SIZE}"
      # Memory configuration
      "-sALLOW_MEMORY_GROWTH=1"
      "-sINITIAL_MEMORY=${cmake_template_WASM_INITIAL_MEMORY}"
    ...
    if(EXISTS "${cmake_template_SHELL_TEMPLATE}")
      ...
      message(FATAL_ERROR "Shell template not found: ${cmake_template_SHELL_TEMPLATE}")
    endif()

    # Copy service worker to target build directory for standalone target builds
    if(EXISTS "${cmake_template_COI_WORKER}")
      add_custom_command(TARGET ${target} POST_BUILD
        COMMAND ${CMAKE_COMMAND} -E copy_if_different
          "${cmake_template_COI_WORKER}"
...
function(cmake_template_create_web_dist)
  if(NOT EMSCRIPTEN)
    return()
  endif()
  ...
  get_property(WASM_TARGETS GLOBAL PROPERTY cmake_template_WASM_TARGETS)
...
")
  ...
  if(EXISTS "${cmake_template_INDEX_TEMPLATE}")
    ...
    message(WARNING "Index template not found: ${cmake_template_INDEX_TEMPLATE}")
  endif()
